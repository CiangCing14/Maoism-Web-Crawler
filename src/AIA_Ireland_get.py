...
l='https://anti-imperialist-action-ireland.com/page/'
l2='https://anti-imperialist-action-ireland.com'
d=str(datetime.today()-timedelta(days=1)).split(' ')[0]
hl=[]
if not os.path.exists('000000.list'):
    for a in range(5):
        h=rg.valid('%s%d'%(l,a+1),'<main id="site-content">').text.split('<main id="site-content">')[1].split('<div class="pagination-wrapper section-inner">')[0]
        h=[b.split('class="entry-title heading-size-1">')[1].split('<a href="')[1].split('"')[0]for b in h.split('<article class="post-')[1:]]
        hl.extend(h)
...
if len(dr)==0:
    ...
    for a in range(len(hl)):
        h=rg.valid(hl[a],'<!-- .entry-header -->').text
        if not os.path.exists('test.txt'):f=open('test.txt','w+');f.write(h);f.close()
        h2=h.split('<!-- .entry-header -->')[1].split('<!-- .post-inner -->')[0]
        h={'title':h.split('<meta property="og:title" content="')[1].split('"')[0].strip(),
           'time':h.split('<meta property="article:published_time" content="')[1].split('"')[0],
           ...
           'text':hp.handle(h2),
           'categories':[b.split('<')[0]for b in h.split('<div class="entry-categories-inner">')[1].split('</div>')[0].split('rel="category tag">')[1:]],
