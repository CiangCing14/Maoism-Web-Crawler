...
l='https://punalippu.noblogs.org/page/'
l2='https://punalippu.noblogs.org'
d=str(datetime.today()-timedelta(days=1)).split(' ')[0]
hl=[]
if not os.path.exists('000000.list'):
    for a in range(5):
        h=rg.rget('%s%d'%(l,a+1)).text.split('<div id="content" role="main">')[1].split('<div id="nav-below" class="navigation">')[0]
        h=[b.split('<a href="')[1].split('"')[0]for b in h.split('<div id="post-')[1:]]
        hl.extend(h)
...
if len(dr)==0:
    ...
    for a in range(len(hl)):
        ...
        h2='</div>'.join(h.split('<div class="entry-content">')[1].split('<div class="entry-utility">')[0].split('</div>')[:-1])
        h={'title':h.split('class="entry-title">')[1].split('<')[0].strip(),
           'time':h.split('<span class="entry-date">')[1].split('<')[0].replace('/','-'),
           'author':h.split('<span class="author vcard">')[1].split('</span>')[0].split('title="View all posts by ')[1].split('">')[1].split('<')[0],
           'images':[html.unescape(b.split('src="')[1].split('"')[0].split('?')[0])for b in h2.split('<img')[1:]],
           'text':hp.handle(h2),
           'categories':[b.split('<')[0]for b in h.split('<div class="entry-utility">')[1].split('</div>')[0].split('rel="category tag">')[1:]],
           'source':hl[a]
          }
