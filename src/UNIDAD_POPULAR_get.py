...
def formalimg(a):
    ...
    for b in t:
        if(b2:='.%s'%b)in a:
            a='%s%s'%(a.split(b2)[0],b2)
    return a

l='https://www.unidadpopular.org.uy/blog/page/'
l2='https://www.unidadpopular.org.uy'
...
if len(dr)==0:
    ...
    for a in range(len(hl)):
        ...
        h4=json.loads(h.split('<script type="application/ld+json">')[1].split('</script>')[0])
        h={'title':h.split('<meta property="og:title" content="')[1].split('"')[0].strip(),
           ...
           'categories':[b.split('</li>')[0].split('data-hook="category-label-list__item">')[1].split('<')[0]for b in h.split(sp)[1:]]if(sp:='<li class="TZ6_II">')in h3 else None,
           'author':'%s (%s)'%(h4['author']['name'],h4['author']['@type']),
           'publisher':'%s (%s)'%(h4['publisher']['name'],h4['publisher']['@type']),
           'images':[html.unescape(formalimg(b.split('src="')[1].split('"')[0].split('?')[0]))for b in h2.split('<img')[1:]if'src="'in b],
           'text':hp.handle(h2),
           'source':hl[a]
          }
        t2=''
