...
l='https://vnd-peru.blogspot.com/'
l2='https://vnd-peru.blogspot.com'
...
if len(dr)==0:
    ...
    for a in range(len(hl)):
        ...
        h2='>'.join('</div>'.join(h.split("<div class='post-body entry-content' id='post-body-")[1].split("<div class='post-footer'>")[0].split('</div>')[:-1]).split('>')[1:])
        h={'time':h.split("<abbr class='published' itemprop='datePublished' title='")[1].split("'")[0],
           ...
           'author':h.split("<span class='post-author vcard'>")[1].split("<span itemprop='name'>")[1].split("<")[0].strip(),
           'images':[b.split('src="'if'"'in b else"src='")[1].split('"'if'"'in b else"'")[0].split('?')[0]for b in h2.split('<img ')[1:]],
           'text':hp.handle(h2),
           'source':hl[a]
          }
