...
l='https://serviraopovo.wordpress.com/page/'
l2='https://serviraopovo.wordpress.com'
d=str(datetime.today()-timedelta(days=1)).split(' ')[0]
hl=[]
if not os.path.exists('000000.list'):
    for a in range(5):
        h=rg.rget('%s%d'%(l,a+1)).text.split('<main id="main" class="site-main" role="main">')[1].split('</main><!-- #main -->')[0]
...
if len(dr)==0:
    ...
    for a in range(len(hl)):
        ...
        h={'title':h.split('<meta property="og:title" content="')[1].split('"')[0].strip(),
           ...
           'author':[b.split('>')[1].split('<')[0]for b in h.split('<span class="author vcard">')[1].split('</span>')[0].split('<a class="')[1:]],
