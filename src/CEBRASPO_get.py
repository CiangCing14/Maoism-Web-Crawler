...
l='https://cebraspo.blogspot.com/'
l2='https://cebraspo.blogspot.com'
...
if len(dr)==0:
    ...
    for a in range(len(hl)):
        ...
        h2=hp.handle(h3:='>'.join(h.split("class='post-title entry-title' itemprop='name'>")[1].split('>')[1:]).split("<div class='post-footer'>")[0]).strip()
        h={'time':h.split("<abbr class='published' itemprop='datePublished' title='")[1].split("'")[0],
           'title':hp.handle(h.split("class='post-title entry-title' itemprop='name'>")[1].split("<")[0].strip()).replace('#','').replace('*','').strip(),
           'author':b if(b:=h.split("<span class='post-author vcard'>")[1].split("<")[0]).strip()else'CEBRASPO',
