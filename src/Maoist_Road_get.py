...
if len(dr)==0:
    ...
    for a in range(len(hl)):
        ...
        h2='</div>'.join('>'.join(h.split("<div class='post-body entry-content' id='post-body-")[1].split('>')[1:]).split("<div class='post-footer'>")[0].split('</div>')[:-1])
        h={'time':h.split("<abbr class='published' itemprop='datePublished' title='")[1].split("'")[0],
           ...
           'description':de if(de:=hp.handle(h.split("' property='og:description'/>")[0].split("'")[-1]).strip())else'None',
